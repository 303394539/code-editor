...
import { paths } from './keywords';
import { searchTokens } from './tokenlizer';
...
const providePathCompletionItems = (lineContent: string) => {
  ...
  const isPathMatch = /^(GET|POST|PUT|DELETE)(\s+[a-zA-Z0-9_\/-?\-&,]*)$/.test(
    lineContent,
  );
...
const getQueryTreePath = (actionBlockContent: string) => {
  ...
  actionBlockContent
    ...
    .forEach((item) => {
      ...
      /[\w.]+:$/.test(pureItem) &&
        pathStack.push(pureItem.split(',').pop() || '');
    });
...
const isReplaceCompletion = (
  lineContent: string,
  textUntilPosition: string,
) => {
  const matches = lineContent
    ?.substring(textUntilPosition.length)
    ?.match(/[,":]/);
...
const searchCompletionProvider = (
  model: monaco.editor.ITextModel,
  position: monaco.Position,
) => {
