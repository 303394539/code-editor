...
export type SQLEditorProps<T extends Mode = any> = Omit<
  EditorProps<T>,
...
export type InternalSQLEditorType = <T extends Mode = 'normal'>(
  props: SQLEditorProps<T> & {
    ref?: Ref<EditorInstance<T>>;
  },
) => ReactElement;

export type SQLEditorInterface = InternalSQLEditorType & EditorStaticInterface;
...
function InternalComponent<T extends Mode = any>(
  { type = 'MySQL', formatOptions, ...props }: SQLEditorProps<T>,
  ref?: Ref<EditorInstance<T>>,
) {
      ...
      disposableList.push(
        monaco.languages.registerCompletionItemProvider(languageMemo, {
          provideCompletionItems(model, position) {
            ...
            const { keywords: newKeywords = [], databases: newDatabase = [] } =
              hintData || {};
            ...
            entities.forEach(({ entityContextType, isAlias, text, alias }) => {
...
const Component = forwardRef(
  InternalComponent,
) as InternalSQLEditorType as SQLEditorInterface;
